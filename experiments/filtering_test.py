...
def propagate(filtering_percent):
    ...
    z, Ap, As = propagation_function(
        ...
        filtering_percent=filtering_percent,
    )
    ...
    return z, signal_power_s, signal_power_p


z, Ps_f, Pp_f = propagate(0.01)
z, Ps, Pp = propagate(0.00)
# %%
...
plt.plot(
    z * 1e-3, 10 * np.log10(Pp_f * 1e3), marker="x", fillstyle="none", label="Filtered"
)
plt.legend()
plt.xlabel("Position [km]")
plt.ylabel("Power [dBm]")
